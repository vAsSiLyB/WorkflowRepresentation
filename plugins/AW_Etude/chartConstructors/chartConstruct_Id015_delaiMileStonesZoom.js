statisticsChart.requestCache['Id015_delaiMileStonesZoom'].initiator = function() {
	...
	var histoData = {
			...
			graphDef : {
				...
				desc : "Délai médian par cases de programmes, zoom sur les 100 derniers jours, Ecart-type hors programmation Eté",
	...
	$.each(histoData.ord.data, function(cat, dataSet) {
		$.each(dataSet, function(codeCase, result) {
			...
			result.median = result[Math.floor(result.length / 2)];
			result.rmsCompute();
