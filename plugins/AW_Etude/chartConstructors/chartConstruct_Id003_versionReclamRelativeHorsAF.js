statisticsChart.requestCache['Id003_versionReclamRelativeHorsAF'].initiator = function() {
	// Histo
	var histoData = {
			id : "repartReclams",
			ord : {
			...
			graphDef : {
				type : "xy",
				subType : "histoMulti",
				title : "Balance des réclamations : Pôles allemands uniquement",
	...
	var listePoles = ["BR", "HR", "MDR", "NDR", "RB", "RBB", "SR", "SWR", "WDR", "ZDF", 
	                  "ARTE PRODUCTIONS"];
		...
		$.each(em.versions, function(index, version) {
			...
			if ((version.fournisseurSon.length && version.fournisseurSon[0] === em.origine)
				...
				if (typeof dataRoot[pole] === 'undefined') {
					...
					dataRoot[pole]['Performance sup 97.5%'] = [];
				}
				if (reclamImage === true) {
...
//	});
	...
	for (var i = 0, l = sortingArray.length; i < l; i++) {
		...
		if (reclamSum * 100 / dataRoot[sortingArray[i]]['Version PAD']['unique'].result < 2.5) {
			dataRoot[sortingArray[i]]['Performance sup 97.5%'] = [];
			dataRoot[sortingArray[i]]['Performance sup 97.5%']['unique'] = [];
			dataRoot[sortingArray[i]]['Performance sup 97.5%']['unique'].result = dataRoot[sortingArray[i]]['Version PAD']['unique'].result;
			dataRoot[sortingArray[i]]['Performance sup 97.5%']['unique'].infoBox = 'Performance > 97.5% : ' + sortingArray[i];
