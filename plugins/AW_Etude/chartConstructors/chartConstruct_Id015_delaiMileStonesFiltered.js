statisticsChart.requestCache['Id015_delaiMileStonesFiltered'].initiator = function() {
	...
	$.each(WG.baseDataRequestCache, function(emNbr, em) {
		...
		$.each(em.versions, function(key, version) {
			if (version.label.slice(0, 2) === 'VA' || version.label.slice(0, 2) === 'VF' || version.label === 'VOA-STF' || version.label === 'VOF-STA') {
	...
	$.each(histoData.ord.data, function(cat, dataSet) {
		$.each(dataSet, function(codeCase, result) {
			result.median();
			result.rmsCompute();
			if (result.rms > 56 || result.length < 6) {
	...
	histoData.emphasizeFunction = function() {
		var graphics = new PIXI.Graphics();
		
//		graphics.drawEllipse(0, 0, )
	}
